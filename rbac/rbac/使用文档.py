...
3. 业务系统中用户表结构的设计
    ...
    优点：用户信息整合到一张表中。所有的用户信息就放在业务的models模型类的表中。用户信息维护便捷。
    缺点：在rbac中所有关于用户表的操作就不能使用了
    注意：
        rbac中有两处使用了rbac的用户表，使用此方式整合用户信息需要做两点改进
            - rbac中用户管理功能 删除
            - 权限分配时的用户列表使用 业务表中的用户表
        需修改rbac组件的代码。

    严重注意：
        在继承后进行数据库迁移时，会找不到role这个表，因为我们写的是字符串，会在当前的model.py中查找，找不到会报错.
        需要将rbac/models.py中的用户表修改为roles = models.ManyToManyField(verbose_name='拥有的所有角色', to=Role, blank=True) 严重提醒 Role 不要加引号
        会将Role这个表的引用继承过去。再进行数据库迁移就不会报错

    对于rbac中代码的修改：
        1.在rbac的url中删除对用户表的url
        2.rbac中使用的用户表修改为业务中的用户表
...
4. 讲业务系统中的用户表的路径写到配置文件。
    ...
    用于在rbac分配权限时，读取业务表中的用户信息。通过配置文件指定业务用户表
